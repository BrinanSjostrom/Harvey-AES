#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>

int getrsboxIndex(int);
unsigned char *subBytes(unsigned char[4][4]);
unsigned char *shiftRows(unsigned char[4][4]);
unsigned char *mixColumns(unsigned char[4][4]);


unsigned char *subBytes(unsigned char table[4][4])
{
  for(int row = 0; row < 4; row++)
  {
    for(int column = 0; column < 4; column++)
    {
      table[row][column] = getrsboxIndex(table[row][column]);
    }
  }
  unsigned char *ptable = &table[0][0];
return ptable;
}

unsigned char *shiftRows(unsigned char table[4][4])
{
  unsigned char buffer = 0x00;
  for(int row = 0; row < 4; row++)
  {
    for(int i = 0; i < row; i++)
    {
      buffer = table[row][0];
      table[i][0] = table[row][1];
      table[i][1] = table[row][2];
      table[i][2] = table[row][3];
      table[i][3] = buffer;
    }
  }

  unsigned char *ptable = &table[0][0];
return ptable;
}

unsigned char *mixColumns(unsigned char table[4][4]) //work on this function!!!!
{
  const unsigned char matrix[4][4] = {{2, 3, 1, 1},
                                      {1, 2, 3, 1},
                                      {1, 1, 2, 3},
                                      {3, 1, 1, 2}};
  unsigned char charColumn[4];
  unsigned char newColumn[4];
  for(int row = 0; row < 4; row++)
  {
    for(int column = 0; column < 4; column++)
    {
      charColumn[column] = table[row][column];
    }
    for(int i = 0; i < 4; i++)
    {
      newColumn[i] = ((matrix[row][0] * charColumn[0]) +
                     (matrix[row][1] * charColumn[1]) +
                     (matrix[row][2] * charColumn[2]) +
                     (matrix[row][3] * charColumn[3])) % 255;
    }
  }
  unsigned char *ptable = &table[0][0];
return ptable;
}

int getrsboxIndex(int num){
  const unsigned int rsbox[256] =
  { };
return rsbox[num];
}
